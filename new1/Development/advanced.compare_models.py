...
# Add value labels
for _idx, (_mean, _std) in enumerate(zip(mean_ginis, std_ginis)):
    ax1.text(_mean + _std + 0.003, _idx, f'{_mean:.4f}', 
...
# Add value labels
for _idx, _imp in enumerate(improvements):
    ax2.text(_imp + 0.5 if _imp > 0 else _imp - 0.5, _idx, f'{_imp:.2f}%', 
             va='center', ha='left' if _imp > 0 else 'right',
